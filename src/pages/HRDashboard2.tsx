...
const HRDashboard = () => {
  ...
  const sendApplicantNotification = async (app: any, status: string) => {
    try {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status, app }: { id: string; status: string; app: any }) => {
      const { error } = await supabase.from("job_applications").update({ status }).eq("id", id);
      if (error) throw error;
      // Auto-send email when shortlisted or hired
      if (status === "shortlisted" || status === "hired") {
        await sendApplicantNotification(app, status);
      }
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["hr-applications"] });
      if (variables.status === "shortlisted") {
        toast({ title: "Status updated", description: `Shortlisting email sent to applicant.` });
      } else if (variables.status === "hired") {
        toast({ title: "Status updated", description: `Selection congratulations email sent to applicant.` });
      } else {
                      ...
                      {isLoading ? (
                        ...
                        filtered.map((app: any) => (
                          <TableRow key={app.id}>
                            ...
                            <TableCell>
                              <Select value={app.status} onValueChange={(val) => updateStatus.mutate({ id: app.id, status: val, app })}>
